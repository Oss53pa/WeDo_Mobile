-- WeDo migration 008 — let a member choose their number of "têtes" when joining.
-- Replaces wedo.rejoindre_tontine(uuid, uuid) with a 3-arg version (p_nb_tetes default 1).
-- The 2-arg callers (e.g. rejoindre_tontine_par_code) keep working via the default.

drop function if exists wedo.rejoindre_tontine(uuid, uuid);
...
begin
  ...
  insert into wedo.tontine_members (tontine_id, user_id, role, status, nb_tetes)
    values (p_tontine_id, auth.uid(), 'Member',
            case when t.auto_approve then 'Active' else 'Pending' end, v_tetes);
